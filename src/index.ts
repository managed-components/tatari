...
export const sendEvent = (key: string) => (event: MCEvent) => {
  ...
  const sessionId = cookie || client.url.searchParams.get('tatari_session_id') || crypto.randomUUID();
  ...
  client.fetch('https://d1lu3pmaz2ilpx.cloudfront.net/5a28e627?' + queryString, {
    credentials: 'include',
    keepalive: true,
    mode: 'no-cors'
  })
